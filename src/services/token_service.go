...
func (s *TokenService) VerifyToken(token string) (*jwt.Token, error) {
	tkn, err := jwt.Parse(token, func(token *jwt.Token) (interface{}, error) {
		...
		if !ok {
			return nil, &service_error.ServiceError{EndUserMessage: service_error.UnExpectedError}
		}
...
func (s *TokenService) GetClaims(token string) (claimMap map[string]interface{}, err error) {
	...
	veryfiToken, err := s.VerifyToken(token)
	if err != nil {
		return nil, err
	}

	claims, ok := veryfiToken.Claims.(jwt.MapClaims)

	if ok && veryfiToken.Valid {
	...
	return nil, &service_error.ServiceError{EndUserMessage: service_error.ClaimsNotFound}
}
