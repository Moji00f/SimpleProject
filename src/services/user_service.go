...
func (s *UserService) RegisterByUsername(req *dto.RegisterUserByUsernameRequest) error {
	...
	if exists {
		return &service_error.ServiceError{EndUserMessage: service_error.EmailExists}
	}
	...
	if exists {
		return &service_error.ServiceError{EndUserMessage: service_error.UsernameExists}
	}
...
func (s *UserService) RegisterLoginByMobileNumber(req *dto.RegisterLoginByMobileRequest) (*dto.TokenDetail, error) {
	...
	if exists {
		...
		err = s.database.
			Model(&models.User{}).
			Where("user_name = ?", u.UserName).Or("mobile_number = ?", u.MobileNumber).
			Preload("UserRoles", func(tx *gorm.DB) *gorm.DB {
				return tx.Preload("Role")
			}).
			Find(&user).Error
